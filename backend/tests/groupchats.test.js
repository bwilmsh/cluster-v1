...
describe('GET /api/groupchats', () => {
    it('lists group chats', async () => {
        const res = await (0, supertest_1.default)(index_1.app).get('/api/groupchats');
...
describe('POST /api/groupchats', () => {
    it('creates a group chat', async () => {
        const res = await (0, supertest_1.default)(index_1.app).post('/api/groupchats').send({ name: 'Test Group', agentIds: [] });
        expect(res.status).toBe(201);
        expect(res.body.name).toBe('Test Group');
    });
    it('returns 400 if name is missing', async () => {
        const res = await (0, supertest_1.default)(index_1.app).post('/api/groupchats').send({});
...
describe('GET /api/groupchats/:id/messages', () => {
    it('returns messages array', async () => {
        const res = await (0, supertest_1.default)(index_1.app).get('/api/groupchats/gc-1/messages');
        expect(res.status).toBe(200);
        expect(Array.isArray(res.body)).toBe(true);
    });
});
