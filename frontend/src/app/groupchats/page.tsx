  ...
  return (
    <div className="flex items-center">
      {agentMembers.map((m, i) => {
        ...
        return (
          <div
            key={m.id}
            title={m.agent!.name}
            className={`w-7 h-7 rounded-full border-2 border-surface-raised flex items-center justify-center text-[10px] font-semibold shrink-0 ${p.bg} ${p.text}`}
      ...
      {members.filter((m) => m.type === 'agent').length > 5 && (
        <div
          className="w-7 h-7 rounded-full border-2 border-surface-raised bg-white/10 flex items-center justify-center text-[10px] text-white/40 shrink-0"
          style={{ marginLeft: -8 }}
        >
          +{members.filter((m) => m.type === 'agent').length - 5}
        </div>
    ...
    <div className="h-full overflow-y-auto">
      <div className="p-8 max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-xl font-semibold text-white">Group Chats</h1>
            ...
            {chats.map((chat) => {
              ...
              return (
                <button
                  ...
                  className="text-left group rounded-xl border border-surface-border bg-surface-raised p-5 hover:-translate-y-0.5 hover:border-white/15 hover:shadow-lg hover:shadow-black/20 transition-all duration-200"
                >
                  {/* Top row */}
                  ...
                  {lastMsg && (
                    <p className="mt-3 text-xs text-white/30 truncate">
                      <span className="text-white/45">{lastMsg.senderName}:</span>{' '}
                      {lastMsg.content}
                    </p>
                  )}

                  {!lastMsg && (
                    <p className="mt-3 text-xs text-white/20">No messages yet</p>
                  )}
                </button>
              )
            })}
